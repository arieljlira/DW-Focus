...
function dw_focus_setup() {
	...
	require_once DW_TEMPLATE_PATH . 'inc/widgets/dw-focus-latest-news.php';
	...
	require_once DW_TEMPLATE_PATH . 'dw-importer/dw-importer.php';
...
if ( ! function_exists('dw_focus_scripts') ) {
	...
	function dw_focus_scripts() {
		wp_enqueue_style( 'dw_focus_template', get_template_directory_uri() . '/assets/css/dw-focus.min.css' );
		wp_enqueue_style( 'dw_focus_responsive', get_template_directory_uri() . '/assets/css/dw-focus-responsive.min.css' );
		...
		wp_enqueue_script('dw_focus', DW_TEMPLATE_URI . 'assets/js/dw-focus.min.js', array( 'jquery' ) );
...
if (!function_exists('dw_focus_public_scripts_conditional')){
	function dw_focus_public_scripts_conditional() {
	?>
		<!--[if lt IE 9]>
	...
	add_action('wp_head', 'dw_focus_public_scripts_conditional');
}
...
if ( ! function_exists('dw_focus_customize_controls_scripts')) {
	function dw_focus_customize_controls_scripts() {
		wp_enqueue_script( 'google_jquery', '//ajax.googleapis.com/ajax/libs/jquery/2.0.0/jquery.min.js' );
		wp_enqueue_script('dw-focus-widgets', DW_TEMPLATE_URI .'assets/admin/js/customize.js', array() );
		wp_enqueue_style('dw-focus-widgets', DW_TEMPLATE_URI .'assets/admin/css/admin-widget.css', array() );
	}
	add_action( 'customize_controls_enqueue_scripts', 'dw_focus_customize_controls_scripts' );
}
...
if( ! function_exists('dw_focus_pagenavi') ) {
	...
	function dw_focus_pagenavi( $the_query = false, $type = false ){
		...
		if( ! $type ) {
			$type = dw_get_option('nav_type','number');get_post_type( get_the_ID() );
		}
		if ( $max > 1 ) { ?>
				<?php if( $type != 'number' && $page_template != 'template-blog.php' && ( strpos($_SERVER['HTTP_USER_AGENT'],'MSIE 8.0') ===false || is_mobile() ) ): ?>
				...
				<?php else: 
					...
					add_filter('paginate_links', 'bug4x');
					...
					<?php foreach( $coauthors as $coauthor ): ?>
						<div class="author-info">
							<?php if( get_the_post_thumbnail( $coauthor->ID ) ) : ?>
								<?php echo get_the_post_thumbnail( $coauthor->ID , array(96,96) ); ?>
							<?php else : ?>
								<?php echo get_avatar( $coauthor->ID, 96); ?>
							<?php endif; ?>
...
if( ! function_exists('dw_gallery_shortcode') ) {
	...
	function dw_gallery_shortcode($attr) {
		...
		foreach ( $attachments as $img_id => $attachment ) {
			...
			$output .= "<span class='gallery-caption'>".$attachment->post_excerpt."</span>";
			$output .= "</div>";
		}
		$output .= " </div>";
...
if( ! function_exists('dw_focus_add_layout_class') ) { 
	function dw_focus_add_layout_class($classes){
		...
		$classes[] = dw_get_option('dw_layout','right-sidebar');
...
if ( function_exists( 'add_image_size' ) ) { 
	...
	update_option( 'medium_size_h', 130 );
...
if( ! function_exists('dw_script_header') ) {
	...
	function dw_script_header() {
		...
		$dw_404_logo = dw_get_option('dw_404_logo','');
...
/*		.dropdown-toggle.loading .read {
		...
		<?php if( ! empty( $dw_404_logo ) )  : ?>
		body.error404 {
			background-image: url(<?php echo $dw_404_logo; ?>);
		}
		<?php endif; ?>
		...
		.btn-primary:hover {
			...
			border-color: <?php echo $dw_custom_color; ?> !important;
		}
 ...
 * Font Selector
 */
if( ! function_exists('dw_typo_font') ) {
  function dw_typo_font(){
	if (dw_get_option('heading_font') && dw_get_option('heading_font') != '') {
	  $heading_font = explode(':dw:', dw_get_option('heading_font') );
	  ?>
	...
	if (dw_get_option( 'body_font') && dw_get_option( 'body_font') != '') {
	  $body_font = explode( ':dw:', dw_get_option( 'body_font' ));
	  ?>
		<style type="text/css" id="body_font" media="screen">
		  ...
		  body, .entry-content, blockquote cite, #under-navigation .dw_focus_headlines .headlines a, #comments #respond #submit, #colophon.dark .btn-danger, input, button, select, textarea, .desktop .top-news .dropdown-menu a {
  ...
  add_filter('wp_head','dw_typo_font');
}
 ...
 */
if( ! function_exists('dw_fbogmeta') ){
	function dw_fbogmeta() {
	  if (is_single()) {
		?>
	...
	add_action('wp_head', 'dw_fbogmeta');
}
...
add_action( 'wp_ajax_dw_top15', 'dw_top15' );
add_action( 'wp_ajax_nopriv_dw_top15', 'dw_top15' );


if( ! function_exists( 'dw_count_lastest_news' ) ){
	function dw_count_lastest_news(){
		global $wpdb;
		...
		$is_news_today = false;
		$news_count = $wpdb->get_var( 'SELECT COUNT(*) FROM '.$wpdb->posts.'  WHERE post_type = "post" AND (post_status = "publish") AND post_date >= "'.date('Y-m-d', strtotime('now') ).'"  ORDER BY post_date DESC LIMIT 0, '.$max_number_posts.' ' );
		if ( $news_count <=0 ) {
				$is_news_today = false;
				$is_news = 'false';
				$display_count = $max_number_posts;
		} else {
				$display_count = $news_count;
				$is_news_today = true;
				$is_news = 'true';
		}
		?>
		<a class="dropdown-toggle" data-max="<?php echo $max_number_posts;?>" data-number="<?php echo $news_count;?>" data-display="<?php echo $display_type; ?>" data-today="<?php echo $is_news;?>" data-toggle="dropdown" href="#">
			<span class="number load"><?php echo $display_count; ?></span>
			<span class="text load" style="display:block"><?php echo $display_type == 'today' && $is_news_today ? __('New Articles today','dw_focus') : __('Must read articles','dw_focus'); ?> <i class="icon-caret-down"></i></span>
		</a>
		<?php
	}
}
