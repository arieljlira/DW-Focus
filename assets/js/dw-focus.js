...
!function ($) {
  ...
  * ============== */
  ...
  $(window).on('load', function () {
	...
	var loading = false, menu_item = false;
	$('.lastest').one('click', function(event) {
			$('.load').removeAttr('style');
			$('.fa-spin').show();
			$('.number').hide();
			$('.lastest').addClass('loading');
			var max = $(this).find('.dropdown-toggle').data('max');
			var number = $(this).find('.dropdown-toggle').data('number');
			var display = $(this).find('.dropdown-toggle').data('display');
			var today = $(this).find('.dropdown-toggle').data('today');
			// $('.dropdown-toggle').append('<i class="fa fa-spinner fa-spin" style="font-size: 60px;"></i>');
			$.ajax({
				...
				success: function( data ){
					$('.top-news').append(data);
					$('.fa-spinner').toggle();
					$('.number').toggle();
					$('.lastest').removeClass('loading');
	...
	// $('.nav > .menu-item-object-category').on( 'mouseover', function() {
	// 	if( ( $(this).find('.sub-menu').length ) && ( ! $(this).find('.sub-mega-wrap').length ) ) {
	// 		console.log('test');
	// 		var $this = $(this);
	// 		cat_ids = [];
	// 		$( $(this).find('.sub-menu > li') ).each( function( index ) {
	// 			cat_ids.push( $(this).attr('class').replace( /.*menu-item-category-(\d+)\s{0,1}.*/, '$1') );
	// 		});

	// 		//cat_id = $(this).find('.sub-menu > li:first-child').attr('id').replace('menu-item-','');
	// 		menu_id = $(this).attr('id');

	// 		if( loading && menu_item != $(this).get(0) ) {
	// 			loading.abort();
	// 		}
	// 		menu_item = $(this).get(0);
	// 		loading = $.ajax({
	// 			type: 'POST',
	// 			dataType: 'json',
	// 			url: dw_focus.ajax_url,
	// 			data: {
	// 				'menu_id': $(this).attr('id'),
	// 				'action': 'dw_focus_submenu_query',
	// 				'cat_ids': cat_ids,
	// 			},
	// 			success: function(data){
	// 				$this.find('.sub-menu').wrap('<div class="sub-mega-wrap"></div>');
	// 				$this.find('.sub-mega-wrap').append('<div class="subcat"><p style="text-align: center">Loading ...</p></div>');
	// 				$( '#' + menu_id ).find('.subcat').html(data.html);
	// 			}
	// 		});
	// 	}
	// });
	...
	// 	if( ( $(this).find('.sub-menu').length ) && ( ! $(this).find('.sub-mega-wrap').length ) ) {
	// 		var $this = $(this);
	// 		var cat_id = $(this).attr('class').replace( /.*menu-item-category-(\d+)\s{0,1}.*/, '$1');
	// 		// cat_ids = [];
	// 		// $( $(this).find('.sub-menu > li') ).each( function( index ) {
	// 		// 	cat_ids.push( $(this).attr('class').replace( /.*menu-item-category-(\d+)\s{0,1}.*/, '$1') );
	// 		// });
