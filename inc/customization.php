...
function dw_focus_customize_register($wp_customize){
    ...
    // $wp_customize->add_setting('option_tree[content_display]', array(
    //     'default' => 'false',
    //     'capability' => 'edit_theme_options',
    //     'type' => 'option',
    ...
    // $wp_customize->add_control( 'content_display', array(
    //     'label'      => __('Display both blog content and home content sidebar ? ', 'dw_focus'),
    //     'section'    => 'dw_focus_general_settings',
    //     'settings'   => 'option_tree[content_display]',
    //     'type' => 'select',
    //     'choices'    => array(
    //         'true'     => __( 'Yes', 'dw_focus' ),
    //         'false'     => __( 'No', 'dw_focus' )
    //     ),
    // ));    
    ...
    $wp_customize->add_control( new WP_Customize_Image_Control($wp_customize, 'dw_logo_image', array(
        'label'    => __('Logo - 230 x 80', 'dw_focus'),
    ...
    $wp_customize->add_control( new WP_Customize_Image_Control($wp_customize, 'dw_small_logo_image', array(
        'label'    => __('Small logo - 100 x 22', 'dw_focus'),
        'section'  => 'title_tagline',
        'settings' => 'option_tree[dw_small_logo_image]'
    )));

    // Logo 404
    $wp_customize->add_setting('option_tree[dw_404_logo]', array(
        'default'               => get_template_directory_uri().'/assets/img/logo-404.png',
        'capability'            => 'edit_theme_options',
        'type'                  => 'option',
    ));
 
    $wp_customize->add_control( new WP_Customize_Image_Control($wp_customize, 'dw_404_logo', array(
        'label'    => __('404 logo - 920 x 200', 'dw_focus'),
        'section'  => 'title_tagline',
        'settings' => 'option_tree[dw_404_logo]'
    )));
    ...
    $wp_customize->add_section('dw_category_settings', array(
        'title'    => __('Archive settings', 'dw_focus'),
        'priority' => 123,
    ));
    ...
    $wp_customize->add_control( new dw_customize_display_style_control($wp_customize, 'cat_display', array(
        'label'      => __('Default archive pages display layout', 'dw_focus'),
    ...
    // New section font selector
    $wp_customize->add_section('dw_font_selector', array(
        'title'    => __('Font Selector', 'dw-focus'),
        'priority' => 120,
    ));
    ...
    $wp_customize->add_setting('option_tree[heading_font]', array(
        'default'        => '',
        'capability'     => 'edit_theme_options',
        'type'           => 'option',
    ));
    $wp_customize->add_control( 'heading_font', array(
        'settings' => 'option_tree[heading_font]',
        'label'   => __('Headding font', 'dw-focus'),
        'section' => 'dw_font_selector',
        'type'    => 'select',
        'choices'    => $newarray
    ));

    $wp_customize->add_setting('option_tree[body_font]', array(
        'default'        => '',
        'capability'     => 'edit_theme_options',
        'type'           => 'option',
    ));
    $wp_customize->add_control( 'body_font', array(
        'settings' => 'option_tree[body_font]',
        'label'   => __('Content font', 'dw-focus'),
        'section' => 'dw_font_selector',
        'type'    => 'select',
        'choices'    => $newarray
    ));
    ...
    $wp_customize->add_setting('option_tree[dw_blog_cat]', array(
        'default'        => dw_get_option('dw_blog_cat'),
        'capability'     => 'edit_theme_options',
        'type'           => 'option',
    ));
    ...
    // Custon code
    $wp_customize->add_section('dw_custom_code', array(
        'title'    => __('Custom code', 'dw_focus'),
        'priority' => 130,
    ));
