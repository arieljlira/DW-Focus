...
<?php
	...
	function upload_sample_image() {
		$url = get_template_directory_uri().'/sample-data/sample.gif';
		$file_name = basename( $url );

		// get placeholder file in the upload dir with a unique, sanitized filename
		$upload = wp_upload_bits( $file_name, 0, '', '2013-01-31 02:33:27' );
		if ( $upload['error'] )
		...
		wp_generate_attachment_metadata( 510, $upload['file'] );
		...
		foreach ( $this->posts as $post ) {
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				...
				if ( 'attachment' != $postdata['post_type'] ) {
					...
					// if( $flag == true ) {
					// 	$url = get_template_directory_uri().'/sample-data/sample.gif';
					// 	$this->fetch_remote_file( $url, $postdata );
					// 	$flag = false;
					// }
	...
	function process_attachment( $post, $url ) {
		...
		if( $url != '' ) {
			...
			$upload['url'] = $upload_dir['baseurl'].'/2013/01/sample.gif';
			$upload['file'] = $upload_dir['basedir'].'/2013/01/sample.gif';
			$this->url_remap[$url] = $upload['url'];
			$this->url_remap[$post['guid']] = $upload['url']; // r13735, really needed?
		}
		...
		$image_arr = array( 
			'width' => 800,
		 	'height' => 800,
		 	'file' => '2013/01/sample.gif',
		 	'sizes' => array( 
		 		'thumbnail' => array( 
		 			'file' => 'sample-110x110.gif', 
		 			'width' => 110,
		 			'height' => 110,
		 			'mime-type' => 'image/gif'),
		 		'medium' => array( 
		 			'file' => 'sample-230x130.gif',
		 			'width' => 230,
		 			'height' => 130,
		 			'mime-type' => 'image/gif' ), 
		 		'large' => array( 
		 			'file' => 'sample-640x360.gif',
		 			'width' => 640,
		 			'height' => 360,
		 			'mime-type' => 'image/gif' ),
		 		'slider-thumb' => array( 
		 			'file' => 'sample-520x400.gif',
		 			'width' => 520, 
		 			'height' => 400, 
		 			'mime-type' => 'image/gif' ),
		 			 ), 
		 	);
		wp_update_attachment_metadata( $post_id, $image_arr );
